...
| I2  | Digital Customer Interfaces employed | Name - Name of the digital customer interface.<br/>Link - URL of a website containing documentation on the DCI.<br/>Mode - In-cloud, on-premise, or other (please describe).<br/>Data exchange standards - Documentation of standards and interfaces used for communication with system operators and other relevant actors.<br/>Note: For each DCI, Procedure 1 must be mapped. |
| I3  | Information about permission administrators in a Member State | Name - Name of the organisation.<br/>Type of identification - ACER, LEI, BIC, EIC, GLN/GS1, or NIC.<br/>Identification of organisation - Code within the selected identification scheme.<br/>Website - Link to the consent administration web application.<br/>User interface - URL or user portal.<br/>Official contact - Contact details for data sharing.<br/>Consent management responsibility for - Metered data administrators covered.<br/>Documentation of access - Explanation of Member State provisions.<br/>Identity service provider - Used for customer authentication.<br/>Eligible party onboarding - Procedure for production environment.<br/>Eligible party test onboarding - Procedure for test environment.<br/>Price list - Costs for eligible parties. |
### Relevant Roles                                                                                                                        
...
|--------:|------|------------------|------------------------------|------------------------------|-----------------------------|
| 1.1 | Execute Digital Customer Interface (pre-condition) | The DCI Operator (final customer or Technical Aggregator) installs and configures the means to communicate directly or indirectly with the CSO. This may be on-premise or cloud-based.<br/>Pre-Condition: The DCI Operator has physical/virtual access to the DCI hardware or service.<br/>Post-Condition: The DCI is operational and can initiate communication with the CSO. | DCI operator | n/a | - |
| 1.2 | Request registration for Digital Customer Interface | During setup, the DCI generates or is assigned a unique identification string. This identifier is provided to the DCI Operator and forwarded to the CSO for certification.<br/>Pre-condition: The DCI is initialized and capable of generating or receiving an identifier.<br/>Post-condition: The CSO has received the DCI identification information. | DCI operator | (Digital) CSO | 1-A – DCI identification information |
| 1.3 | Validate DCI Registration Request | CSO validates that the provided DCI identifier belongs to the final customer and/or to the DCI Operator. This includes checking data consistency and ensuring that contractual and master data are up to date. If applicable, the SO may issue digital credentials and certificates (e.g. PKI/X.509) to secure further interactions. The CSO may employ authentication via eIDAS eID and eIDAS Trust Services (for public/private key creation and timestamp synchronisation). If validation fails, an error flow is triggered providing a meaningful indication to the requester.<br/>Pre-condition: The CSO has received the DCI identification information.<br/>Post-condition: The DCI is validated and, if successful, certified credentials are issued; or an error message is sent to the requester. | (Digital) CSO | DCI operator | 2-B – DCI certification information |
| 1.4 | Execute the installation of the certified credential information (post condition) | The DCI Operator installs the certified credential information (e.g. CSO-signed authentication certificate) into the DCI.<br/>Pre-condition: The DCI Operator has received valid certified credential information from the CSO.<br/>Post-condition: The DCI contains valid, certified credential information, enabling secure communication with the CSO. | DCI operator | DCI operator | - |

| Information exchanged, ID | Name of information | Description of information exchanged |
|--------------------------|---------------------|--------------------------------------|
| 1-A | DCI identification information | DCI identifier - Identifier used by the DCI in further communication<br>Certification request - If applicable, a certification request string to be signed by the CSO as a trusted authority<br>Identifier of the final customer - Final customer identification to be understood by the CSO<br>Identifier of the CUO - If applicable and if different from the final customer, the identification of the CUO |
| 1-B | DCI certification information | DCI identification information - 1-A provided for validation<br>Confirmation result - Result of the step, e.g. to indicate success or invalid data<br>Signed certificate - If applicable, the signed certificate for the PKI keypair<br>CSO DCI identification - Identification of the DCI as registered at the CSO (for further communication) |
...
| 2.2 | Execute the connection application study | CSO processes the connection request and defines the proposal to the Final Customer: a connection with firm capacity matching the customer request;<br/>or 1 or several proposals for flexible connection agreements specifying the FCA conditions (if applicable);<br/>or refusal of connection (if applicable). | CSO                                                                    |              |              |                                          |
| 2.3      | Request to sign connection proposal                                  | CSO sends the connection proposal to the Final customer Note: a « regular » connection proposal is consistent with 2-B FCA condition where flexible capacity is 0, time block is «always», and agreed duration of transitory phase is 0 | CSO             | Final Customer   | 2-B FCA conditions                      |
| 2.4      | Sign the connection proposal |The Final Customer: accepts the connection proposal;<br/>or rejects its connection request (it can then restart the process and apply for a different connection).<br/><br/> If the proposal consists in firm connection agreement, then end of Harmonized Role Model description in this procedure, if no need for CU control or real-time access by CSO <br/><br/>If Flexible Connection Agreement or need for CU control or real-time access, process follows next steps                                                                         | Final customer             | CSO   | 2-C-Acceptance of connection proposal                                         |
...
| 2.7      | Request CUO to command and control its flexible assets                           | The final customer contacts the CUO of its choice and forward data on its FCA conditions Note: A «regular» connection proposal is consistent with 2-B FCA condition where flexible capacity is 0, time block is « always », and agreed duration is 0  | Final customer             | CUO   | 2-B FCA conditions                       |
| 2.8      | Send the service agreement for CU operation | The CUO sends to the Final Customer a proposal for service provision <br/> <br/> The service agreement for CU operation includes at least: consent for data sharing (data with CUO and data to be forwarded to the eligible parties) and command and control by CUO,<br/>mandate (representation contract) for CUO to interact with CSO on its behalf,<br/>Service level agreement,<br/>Other Terms and Conditions                                                                                     | CUO   | Final customer             | 2-F- service agreement for CU operation                     |
...
| Information exchanged, ID | Name of information | Description of information exchanged | 
|----|----|----|
| 2-A | Connection application | Power<br/>(Optional) Type of CU<br/>(Optional) Connection Point identifier<br/>(Optional) Requested power per time block | 
| 2-B | FCA conditions | Maximum capacity (in both directions (guaranteed and flexible) by time block<br/>Agreed duration (expected date for granting connection to the entire requested firm capacity or permanent FCA)<br/>Applicable Network charges<br/>(optional): Minimum intake/outtake when FCA is activated<br/>(Optional) Additional contractual information<br/>(Optional) Need for near real-time data<br/>(Optional) need for forecast of power intake / outtake<br/>Safe-state setpoint (minimum guaranteed connection capacity) in case of loss of communication | 
| 2-C | Acceptance of connection proposal | Y / N |
| 2-D | Near real time data needed by CSO | Request for technical data (referred to procedure 5 information object 5-B) specifying at least latency and periodicity |
| 2-E | Customer consent for near real time data needed by CSO | Y / N |
| 2-F | Service agreement for CU operation | Consent for data sharing (data with CUO and data to be forwarded to eligible parties) and consent for command and control by CUO<br/>Mandate (representation contract) for CUO to interact with CSO on behalf of the Fina Customer.<br/>Service level agreement<br/>Other Terms and Condition | 
| 2-G | Consent for service agreement for CU operation | Consent (Y/N) |
| 2-H | Chosen CUO | ID of CUO |
| 2-I | Consent for service agreement for CU operation | List of data associated with customer consent |
| 2-J | FCA technical specifications | 2-B + 2-D<br/>Technical parameters such as latency, frequency<br/>Connectivity information (such as IP address, ) |
| 2-K | Operational readiness notification | Ready |
| 2-L | Information to prepare for the activation test|To be defined
| 2-M | Activation test result confirmation | Successful / Failed<br/>Authorized / |
| 2-N | FCA Commissioning authorization | Authorized firm capacity<br/>Authorized Flexible capacity |
...
| 3.3      |  (optional) Send new connection conditions                                | CSO informs the final customer about the new (fallback) connection agreement Note: The fallback conditions may have been defined at the time of the FCA proposal| CSO             | Final Customer   | 3-C New (fallback) contractual conditions: same as 2-B except no information on agreed duration                     |
...
| Information exchanged, ID | Name of information | Description of information exchanged | 
|----|----|----|
| 3-A |Revocation request | Capacity (in both directions) by time block<br/>(Optional) Type of CU<br/>Connection Agreement Point identifier | 
| 3-B |Revocation validation | Consent (Y/N) | 
| 3-C | New (fallback) contractual conditions | Capacity (in both directions) by time block<br/>Applicable Network charges<br/>(Optional) Additional contractual information<br/>(Optional) need for near real-time data<br/>(Optional) Need for forecast of power intake / outtake<br/>Safe state setpoint (minimum guaranteed connection capacity) in case of loss of communication<br/>(optional) alternatives<br/>Same as 2B excepts no information on agreed duration|
...
| Information exchanged, ID | Name of information | Description of information exchanged | 
|----|----|----|
| 4-A | Termination information | Information object | 
| 4-B | New (fallback) contractual conditions | Same as 3-C<br/>Capacity (in both directions (guaranteed) by time block<br/>Applicable Network charges<br/>(Optional) Additional contractual information<br/>(Optional) Need for near real-time data<br/>(Optional) need for forecast of power intake / outtake<br/>(Optional) alternatives<br/>(Optional) Type of CU<br/>Optional) Connection Agreement Point identifier | 
| 4-C | Information object | Same as 4-E |
| 4-D | Technical specifications of Connection Agreement | same as 2-J and 3-E:<br/>Capacity (in both directions (guaranteed) by time block)<br/>Applicable Network charges<br/>(Optional) Additional contractual information<br/>(Optional) Near real-time needed data<br/>(Optional) Forecast of power intake / outtake<br/>(Optional) alternatives<br/>(Optional) Type of CU<br/>(Optional) Connection Agreement Point identifier |
...
| 5.8 | Execute the allocation of activation / limitation per site & CU, and the computation of the forecasted impact (power intake / outtake) per Final Customer or ensemble of Final Customers | The CUO computes (or updates) The activation or limitation per site and per CU to comply with the activation / limitation of a flexibility event per Final Customer or ensemble of Final Customers<br/>The forecasted impact (power intake / outtake for each Final Customer or ensemble of Final Customers (FCA, SPU, SPG or part of SPG)) | CUO |  |  |
...
| Information exchanged, ID | Name of information | Description of information exchanged |
|----|----|----|
| 5-A | Forecasted baseline intake / outtake | Time series of active / reactive power in both directions |
| 5-B | Near real time intake / outtake | Actual power (active / reactive) |
| 5-C | Limit notification or activation per FC, or over an ensemble of FCs | Maximum Active / reactive Power in one or both directions<br/>Time frame (start time / end time)<br/>(Optional) Minimum active / reactive power in one or both directions<br/>(Optional) Legal or contractual provision pursuant to the limitation or activation<br/>Identification of the FC, or the ensemble of FCs |
| 5-D | (Optional) Limitation / activation receipt notification of flexibility event | (Optional): yes, I have received and can comply<br/>(Optional): yes, I have received and cannot comply |
| 5-E | CU limit / activation notification at CU level | Maximum and/or minimum Active / reactive Power in one or both directions<br/>Time frame (start time / end time) |
| 5-F | (Optional) Limitation / activation receipt notification at CU level | (Optional): yes, I have received and can comply<br/>(Optional): yes, I have received and cannot comply |
